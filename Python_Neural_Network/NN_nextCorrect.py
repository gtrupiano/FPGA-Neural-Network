...
class NeuralNetwork:
    ...
    def forward_pass(self, x, y, last_r, last_g, last_b):
        input_vector = np.array([x,y,last_r,last_g,last_b]).reshape(-1, 1)
...
input_size = 5
model = NeuralNetwork(input_size, 20 ,3, 3)
    ...
    for p in range(10): 
        ...
        for j in range(10):
            for i in range(10):
                ...
                predicted_r, predicted_g, predicted_b = model.forward_pass((x/10)+0.5, (y/10)+0.5, last_predicted_r, last_predicted_g, last_predicted_b)
                last_predicted_r, last_predicted_g, last_predicted_b = image_array_normal[x][y][0], image_array_normal[x][y][1], image_array_normal[x][y][2]
