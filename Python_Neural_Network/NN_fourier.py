...
class Neuron:
    def __init__(self, input_size):
        self.weights = abs(np.random.randn(input_size, 1) * 0.1).tolist()
        self.bias = 0.0000001
        self.output = None
        self.sensitivity = 0
        self.input = None
...
class NeuralNetwork:
    ...
    def forward_pass(self,x , y, fourier_featureX1, fourier_featureX2, fourier_featureX3, fourier_featureX4, fourier_featureY1, fourier_featureY2, fourier_featureY3, fourier_featureY4):
        input_vector = np.array([x, y, fourier_featureX1, fourier_featureX2, fourier_featureX3, fourier_featureX4, fourier_featureY1, fourier_featureY2, fourier_featureY3, fourier_featureY4]).reshape(-1, 1)
    ...
    def backward_pass(self, r, g, b, target_r, target_g, target_b):
        ...
        error_r =  math.sqrt((r - target_r) **2)
        error_g = math.sqrt((g - target_g) **2)
        error_b = math.sqrt((b - target_b) **2)
...
input_size = 10
model = NeuralNetwork(input_size, 50 ,3, 3)
...
image = "C:\\Users\\JoeyV\\OneDrive\\4510\\Projects\\augmented-reality-master (1)\\augmented-reality-master\\NN\\face.png"
...
while k < 10000:
    ...
    for p in range(1):
        for j in range(10):
            for i in range(10):
                y = (i / 10) * (2 * math.pi) - math.pi
                x = (j / 10) * (2 * math.pi) - math.pi
                fourier_featureX = [math.sin(x), math.cos(x), math.sin(2*x), math.cos(2*x)]
                fourier_featureY = [math.sin(y), math.cos(y), math.sin(2*y), math.cos(2*y)]
                # print("inputs: ", x, y, fourier_featureX[0], fourier_featureY)
                predicted_r, predicted_g, predicted_b = model.forward_pass(x,y,fourier_featureX[0],fourier_featureX[1],fourier_featureX[2],fourier_featureX[3], fourier_featureY[0],fourier_featureY[1],fourier_featureY[2],fourier_featureY[3])
                model.backward_pass(predicted_r, predicted_g, predicted_b, image_array_normal[i][j][0], image_array_normal[i][j][1], image_array_normal[i][j][2])
                # print("x, y", x, y)
                # print(image_array_normal[x][y][0])
                try:
                    image_output[i][j] = [predicted_r[0]*255, predicted_g[0]*255, predicted_b[0]*255,255]
