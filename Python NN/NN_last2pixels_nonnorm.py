...
class Neuron:
    def __init__(self, input_size):
        self.weights = abs(np.random.randn(input_size, 1) * 0.00).tolist()
    ...
    for p in range(1): 
        ...
        for j in range(10):
            for i in range(10):
                ...
                predicted_r, predicted_g, predicted_b = model.forward_pass(x, y, last_predicted_r[0], last_predicted_g[0], last_predicted_b[0],last_predicted_b2[0], last_predicted_g2[0], last_predicted_b2[0])
                ...
                model.backward_pass(predicted_r, predicted_g, predicted_b, image_array[x][y][0], image_array[x][y][1], image_array[x][y][2])
                # print("x, y", x, y)
                # print(image_array_normal[x][y][0])
                try:
                    image_output[x][y] = [predicted_r[0], predicted_g[0], predicted_b[0],255]
