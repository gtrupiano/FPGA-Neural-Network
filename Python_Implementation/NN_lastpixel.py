...
class Neuron:
    def __init__(self, input_size):
        self.weights = abs(np.random.randn(input_size, 1) * 0.01).tolist()
        self.bias = 0.0000001
        self.output = None
        self.sensitivity = 0
        self.input = None
    ...
    for p in range(1): 
        ...
        for j in range(10):
            for i in range(10):
                ...
                predicted_r, predicted_g, predicted_b = model.forward_pass((x/10)+0.5, (y/10)+0.5, last_predicted_r[0], last_predicted_g[0], last_predicted_b[0])
